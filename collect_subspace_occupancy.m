...
% Root Path - 
	...
	occupancy_lfp_filename = 'occupancy_time_control.mat';

	total_folders = length(animal_folders_list);

	subspace_occupancy_across_animals_csd = [];
	subspace_occupancy_across_animals_lfp = [];
...
% Core Loop
	...
	for animal_iterator = 1:total_folders
		...
		subspace_occupancy_across_animals_lfp = [subspace_occupancy_across_animals_lfp ; global_subspace_occupancy_fraction_lfp];

		fig_lfp = 1 ;
		figure(fig_lfp)
		% set(gcf, 'Position', get(0, 'Screensize'));
		subplot(1,2,1)
		plot(global_subspace_occupancy_fraction_lfp, 'Color', animal_colors(animal_iterator,:),'lineWidth', 2,'HandleVisibility','off')
		hold on
		scatter(1:4, global_subspace_occupancy_fraction_lfp, 100, animal_colors(animal_iterator,:), 'filled')
		title('LFP')
		hold on
		...
		subspace_occupancy_across_animals_csd = [subspace_occupancy_across_animals_csd ; global_subspace_occupancy_fraction_csd];

		fig_lfp = 1 ;
		figure(fig_lfp)
		subplot(1,2,2)
		plot(global_subspace_occupancy_fraction_csd, 'Color', animal_colors(animal_iterator,:),'lineWidth', 2,'HandleVisibility','off')
		hold on
		scatter(1:4, global_subspace_occupancy_fraction_csd, 100, animal_colors(animal_iterator,:), 'filled')
		title('CSD')
		hold on
