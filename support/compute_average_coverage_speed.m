% ===================================================================
% Compute Average Speed of Coverage
...
% Root Path - 
	...
	correlate_lfp_folder = 'correlate_lfp_time';

	correlate_lfp_filename = 'corr_vector_across_trials_lfp_over_time.mat';
	...
	for animal_iterator = 1:total_folders
		...
		% -------------------------------------------------------------------	
			...
			for trial_iterator = 1:total_trials		
				...
				for time_iterator = 1 : total_time_points
					...
					lfp_trials_across_animals = [lfp_trials_across_animals;  current_time_matrix];
			...
			[nonrem_cmap, nonrem_colors] = assign_colors(global_nonrem_values, nonrem_range, 10, 'cividis');
		...
		% -------------------------------------------------------------------	
			...
			[umap_output_lfp, umap_params] = run_umap(lfp_trials_across_animals, 'n_components', n_components, 'n_neighbors', 5 ,'min_dist', 0.1, 'metric', 'cosine' );
			x = umap_output_lfp(:,1);
			y = umap_output_lfp(:,2);
			if n_components > 2
				z = umap_output_lfp(:,3);
			...
			title(sprintf('%s Sleep Status Overlay on lfp CorrMat over time',animal_id))
			pbaspect([1 1 1])
			legend('Awake','Sleep')



			fname = sprintf('%s_sleep_status_overlay_lfp_corr_mat_across_trials_raw.png', animal_id);
			...
			title(sprintf('%s lfp CorrMat over time',animal_id))
			pbaspect([1 1 1])

			fname = sprintf('%s_lfp_corr_mat_across_trials_raw.png', animal_id);
			lfp_filepath = fullfile(output_folder, fname);
			saveas(gcf, lfp_filepath);
			...
			suptitle(sprintf('%s  REM and NonREM overlay on lfp CorrMat over time',animal_id))

			fname = sprintf('%s_lfp_corr_mat_across_trials.png', animal_id);
			lfp_filepath = fullfile(output_folder, fname);
			saveas(gcf, lfp_filepath);
		...
		% -------------------------------------------------------------------	
			...
			% 	trial_x = umap_output_lfp(trial_timeframe_range,1);
			% 	trial_y = umap_output_lfp(trial_timeframe_range,2);
