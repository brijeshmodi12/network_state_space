...
scatter3(x_data, y_data, z_data, 15, color_gray, 'filled');
pbaspect([1 1 1]) 
hold on
...
for n = 2:1:length(x_data)
    ...
    if n <= blockSize
        ...
        n_start = n_start + no_of_points;
        n_stop = n_start + no_of_points;
    end
    ...
    % p1;
    plot3(x_data(n_start:n_stop), y_data(n_start:n_stop), z_data(n_start:n_stop) ,tailFormat);
    scatter3(x_data(n_stop), y_data(n_stop),  z_data(n_stop), 50, head_color, 'filled');

    current_time = n * no_of_points * timeStep ;
